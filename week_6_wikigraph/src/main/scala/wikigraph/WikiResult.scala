...
case class WikiResult[A](value: Future[Either[Seq[WikiError], A]]):
  ...
  def map[B](f: A => B)(using ExecutionContext): WikiResult[B] =
    val transformationFn = this.value.map { result =>
      result.map(successValue => f(successValue))
    }
    WikiResult(transformationFn)
  ...
  def flatMap[B](f: A => WikiResult[B])(using ExecutionContext): WikiResult[B] =
    val futureB: Future[Either[Seq[WikiError], B]] = this.value.flatMap {
      case Left(seqOfErrors)       => Future(Left(seqOfErrors))
      case Right(successValueThis) => f(successValueThis).value
    }
    WikiResult(futureB)
...
object WikiResult:
  ...
  def traverse[A, B](as: Seq[A])(f: A => WikiResult[B])(using
    ...
    as.foldLeft[WikiResult[Seq[B]]](WikiResult.successful(Seq.empty)) {
      (accValues, nextElement) =>
        accValues.zip(f(nextElement)) map { (zippedItemsAcc, zippedItem) =>
          zippedItemsAcc :+ zippedItem
        }
    }
