...
trait DecoderInstances:
  ...
  given intDecoder: Decoder[Int] = 
    Decoder.fromPartialFunction { case Json.Num(bigDecimalValue) if bigDecimalValue.isValidInt =>  bigDecimalValue.toInt }
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      decoder.decode(item) flatMap { decodedItem =>
        Some(decodedItem +: previouslyDecodedItems)
      }
    ...
    def processItem(item: Json, maybePreviouslyDecodedItems: Option[List[A]]): Option[List[A]] =
      maybePreviouslyDecodedItems flatMap { prevDecodedItems =>
        decodeAndPrepend(item, prevDecodedItems)
      }
    ...
    Decoder.fromFunction { 
      case possibleJsonArray: Json.Arr => 
        decodeAllItems(possibleJsonArray.items)
      case _ => 
        None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction { 
      case possibleJsonObject: Json.Obj =>
        possibleJsonObject.fields.get(name).flatMap(decoder.decode)
      case _ => 
        None
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] = 
    Decoder.field[String]("name")
      ...
      .transform[Person]((userName, userAge) => Person(userName, userAge))
